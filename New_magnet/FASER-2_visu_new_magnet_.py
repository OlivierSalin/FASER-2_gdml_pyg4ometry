...
def SciFiTracker(vis= False):
    ...
    y_or = gdml.Constant("y_or",2000 , reg)
    x_or = gdml.Constant("x_or",2000 , reg)
    ...
    SciFiLv_hor_ds=SF_LV["SciFiLv_hor_ds"]
    Fiberm=SF_LV["Fiberm"]
...
#downstream
    ...
    for j in range(3,nLayer_ds+3):
        ...
        SciFi_Trk_ds_horizPV = pyg4ometry.geant4.PhysicalVolume([0,0,0],[x_or,y_or, d1_ds+j*d_layers+d_hor_vert], SciFiLv_hor_ds,"SciFi_Trk_ds_horizPV"+ str(j),worldLV,reg)
...
#New SC magnet
    ...
    SC_mag_dim_out_diam=SC_mag_dim_in_diam+400
    ...
    for k in range(0,n_SCmag):
        ...
        SCmag_main_pv= pyg4ometry.geant4.PhysicalVolume([0,0,0],[x_or,y_or, z_SCmag+k*SC_mag_z+k*SC_mag_gap], SCmag_main_lv,"SCmag_main_pv"+ str(k),worldLV,reg)

        #In between tracker
        # SciFi_Trk_us_vertPV_mag = pyg4ometry.geant4.PhysicalVolume([0,0,((-1)**j)*_np.pi/180],[x_or,y_or, z_SCmag+(k+0.5)*SC_mag_z+(k+0.35)*SC_mag_gap], SciFiLv_vert_us,"SciFi_Trk_us_vertPV_mag"+str(k),worldLV,reg)
        # SciFi_Trk_us_horizPV_mag = pyg4ometry.geant4.PhysicalVolume([0,0,0],[x_or,y_or, z_SCmag+(k+0.5)*SC_mag_z+(k+0.35)*SC_mag_gap+d_hor_vert], SciFiLv_hor,"SciFi_Trk_us_horizPV_mag"+ str(k),worldLV,reg)
    ...
    Veto_pv = g4.PhysicalVolume([0, 0, 0], [x_or, y_or, z_veto], VetoLv, "Veto_pv",worldLV,reg)
    ...
    w.addDetector(reg)
    w.write("FASER2_new_magnet_2_Trk_bef_aft.gdml")
...
def SciFiTrackerLayer(reg = None) :
    ...
    bx_vert_us = pyg4ometry.gdml.Constant("bx_vert_us",Dim_SF,reg,True)
    by_vert_us = pyg4ometry.gdml.Constant("by_vert_us",Dim_SF,reg,True)
    bz_vert_us = pyg4ometry.gdml.Constant("bz_vert_us","40",reg,True)

    bx_vert_ds = pyg4ometry.gdml.Constant("bx_vert_ds",Dim_SF_ds,reg,True)
    by_vert_ds = pyg4ometry.gdml.Constant("by_vert_ds",Dim_SF,reg,True)
    bz_vert_ds = pyg4ometry.gdml.Constant("bz_vert_ds","40",reg,True)

    bx_hor = pyg4ometry.gdml.Constant("bx_hor",Dim_SF+100,reg,True)
    by_hor = pyg4ometry.gdml.Constant("by_hor",Dim_SF-100,reg,True)

    bx_hor_ds = pyg4ometry.gdml.Constant("bx_hor_ds",Dim_SF_ds+150,reg,True)
    by_hor_ds = pyg4ometry.gdml.Constant("by_hor_ds",Dim_SF-150,reg,True)
    ...
    SciFi_Trk_veto = pyg4ometry.geant4.solid.Box("SciFi_Trk_veto", bx_vert_us, by_vert_us, bz_vert_us+100,reg)
    ...
    SciFi_Trk_hor_ds   = pyg4ometry.geant4.solid.Box("SciFi_Trk_hor_ds", bx_hor_ds, by_hor_ds, bz_hor,reg)
    SciFiLv_hor_ds = pyg4ometry.geant4.LogicalVolume(SciFi_Trk_hor_ds,Fiberm,"SciFiLv_hor_ds",reg)

    MuonLv_hor = pyg4ometry.geant4.LogicalVolume(SciFi_Trk_hor_ds,"G4_Si","MuonLv_hor",reg)

    VetoLv = pyg4ometry.geant4.LogicalVolume(SciFi_Trk_veto,"G4_PLASTIC_SC_VINYLTOLUENE","VetoLv",reg)
    
    SF_LV["SciFiLv_hor"]=SciFiLv_hor
    SF_LV["SciFiLv_hor_ds"]=SciFiLv_hor_ds
    SF_LV["Fiberm"]=Fiberm
